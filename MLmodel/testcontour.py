...
###################################################################################################
def main():
    imgTrainingNumbers = cv2.imread("AllThaiChars2.jpg")            # read in training numbers image
    ...
    npaContours, npaHierarchy = cv2.findContours(imgThreshCopy,        # input image, make sure to use a copy since the function will modify this image in the course of finding contours
                                                 ...
                                                 cv2.CHAIN_APPROX_TC89_KCOS)           # compress horizontal, vertical, and diagonal segments and leave only their end points
    ...
    intClassifications = []         # declare empty classifications list, this will be our list of how we are classifying our chars from user input, we will write to file at the end
                                    #กขฃคฅฆงจฉชซฌญฎฏฐฑฒณดตถทธนบปผฝพฟภมยรฤลฦวศษสหฬอฮฯะาเโใไๅๆ
                                    # possible chars we are interested in are digits 0 through 9, put these in list intValidChars
    intValidChars = [ord('ก'), ord('ข'), ord('ฃ'), ord('ค'), ord('ฆ'), ord('ง'), ord('จ'), ord('ฉ'), ord('ช'), ord('ซ'),
                     ord('ฌ'), ord('ญ'), ord('ฎ'), ord('ฏ'), ord('ฐ'), ord('ฑ'), ord('ฒ'), ord('ณ'), ord('ด'), ord('ต'),
                     ord('ถ'), ord('ท'), ord('ธ'), ord('น'), ord('บ'), ord('ป'), ord('ผ'), ord('ฝ'), ord('พ'), ord('ฟ'),
                     ord('ภ'), ord('ม'), ord('ย'), ord('ร'), ord('ฤ'), ord('ล'), ord('ฦ'), ord('ว'), ord('ศ'), ord('ษ'), ord('ส'),
                     ord('ห'), ord('ฬ'), ord('อ'), ord('ฮ'), ord('ฯ'), ord('ะ'), ord('า'), ord('เ'), ord('โ'), ord('ใ'), ord('ไ'), ord('ๅ'), ord('ๆ')]
